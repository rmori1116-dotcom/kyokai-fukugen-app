...
function recStake(p){ return (p&&p.rec&&p.rec.s)||''; }
function progCounts(){
  let da=0, ki=0, no=0;
...
function setRec(id, kind, stake){
  ...
  applyRec(p, { k:kind, d:todayStr(), w:(store.settings.worker||'').trim(),
                s:stake||currentStake(), t:Date.now() });
  const c = progCounts();
  toast(`${p.name} を ${ST_NAME[kind]} にしました（打設${c.da}／既設${c.ki}）`);
}
...
function draw(){
  ...
  if(store.layers.pt!==false){
    ...
    for(const p of store.points){
      ...
      if(dm==='name') labelNoOverlap(x, y-pr-6, p.name, k==='no'?'#5a6068':col, nf);
    }
  }
  /* --- 距離（線の上に、線に沿って）---
...
function routeLabel(r){
  const idx=ptIndex();
  return (r.ptIds||[]).map(id=>{ const p=idx.get(id); return p?p.name:'?'; }).join('－');
}
...
function renderRec(){
  ...
  document.getElementById('recTitle').textContent = p.name;
  ...
  const row = p=>{
    ...
    return `<div class="listRow">
      ...
      <span style="flex:1;min-width:0;">
        <b>${esc(p.name)}</b> <span class="stChip ${k}">${ST_NAME[k]}</span>
...
function parseCsvText(text){
  ...
  for(const raw of text.split(/\r?\n/)){
    ...
    if(!isFinite(X)||!isFinite(Y)) continue;
    const Z=parseFloat(f[3]);
    pts.push({ no:String(pts.length+1), name:(f[0]||'').trim()||String(pts.length+1),
               X, Y, z:isFinite(Z)?Z:0, stake:(f[5]||'').trim() });
  }
  return { pts, plots:[] };
}
/* SIMAかCSVかの判定。
